...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/" mode="xmlverb">
        ...
        <html>
            <head>
                <title>WS Viewer</title>
                <!--<link rel="stylesheet" type="text/css" href="/moving/assets/xmlverbatim.css"/>-->
                <style>
    ...
    <xsl:template name="xmlverb-attrs">
        ...
        <span class="xmlverb-attr-name">
            <xsl:choose>
                <xsl:when test="$resources:{(name() = '$it$')}; separator=' or '$">
                    <xsl:element name="a">
                        <xsl:attribute name="href">
                            <xsl:text>/moving/ws</xsl:text>
                            <xsl:value-of select="."/>
                        </xsl:attribute>
